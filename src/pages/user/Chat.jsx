...
export default function UserChatPage() {
   ...
   const activeDriverOption = useMemo(() => {
      if (!meta?.driverOptions?.length || !selectedItemId) return null;
      return meta.driverOptions.find(opt => String(opt.orderItemId) === String(selectedItemId));
   }, [meta, selectedItemId]);
   ...
   const fetchOrders = async () => {
      ...
      try {
         ...
         if (response.data?.success) {
            ...
            } else if (!eligibleOrders.some(order => String(order._id) === String(selectedOrderId))) {
   ...
   useEffect(() => {
      ...
      } else if (!selectedItemId && meta.driverOptions?.length) {
         setSelectedItemId(meta.driverOptions[0].orderItemId);
      }
   }, [meta, selectedItemId]);
   ...
   return (
      <div className="p-4 md:p-6 space-y-6">
         <div className="flex items-center justify-between">
            <div>
               <h1 className="text-2xl font-semibold text-blue-900">Chat với tài xế</h1>
               <p className="text-gray-500">Trao đổi với tài xế sau khi đơn đã có người nhận.</p>
            </div>
            <Button
               icon={<ReloadOutlined />}
               onClick={fetchOrders}
         ...
         <div className="flex flex-col lg:flex-row gap-6">
            <div className="lg:w-1/3 space-y-3">
               <h2 className="text-lg font-semibold text-gray-800">Đơn có thể chat</h2>
               {ordersError && (
                  <Alert
                     type="error"
                     showIcon
               ...
               {ordersLoading ? (
                  ...
                  <div className="space-y-3">
                     {orders.map(order => {
                        const driverPreview = order.items?.find(item => item.driverId);
                        return (
                           <button
                              key={order._id}
                              type="button"
                           ...
                           >
                              ...
                              <p className="text-sm text-gray-600 mt-2">
                                 Tài xế:{' '}
                                 {driverPreview?.driverId?.userId?.name || 'Chưa rõ'}
                              </p>
                           </button>
                        );
                     })}
                  ...
                  <Empty description="Chưa có đơn nào có thể chat" />
               )}
            </div>
                     ...
                     <>
                        <div className="border-b pb-4 mb-4">
                           <div className="flex flex-wrap items-center gap-3 justify-between">
                              <div>
                                 <p className="text-sm text-gray-500">Đang chat với</p>
                                 <p className="text-lg font-semibold text-gray-900">
                                    {activeDriverOption?.driverName || 'Tài xế chưa cập nhật'}
                                 </p>
                              ...
                              <div className="text-right">
                                 ...
                                 {activeDriverOption?.driverPhone && (
                           ...
                           {meta?.driverOptions?.length > 1 && (
                              <div className="mt-4">
                                 ...
                                 >
                                    {meta.driverOptions.map(option => (
                                       <Select.Option
                                          key={option.orderItemId}
                                          value={option.orderItemId}
                                       >
                                          #{String(option.orderItemId).slice(-5)} • {option.driverName || 'Tài xế'} ({option.status})
                                       </Select.Option>
                                    ))}
                                 </Select>
                              </div>
                        ...
                        <div className="flex-1 flex flex-col min-h-[420px]">
                                 ...
                                 messages.map(msg => {
                                    ...
                                    return (
                                       ...
                                       >
                                          ...
                                          <div className={`text-xs mt-1 opacity-75 ${isCustomer ? 'text-blue-100' : 'text-gray-500'}`}>
                           ...
                           <div className="border-t pt-3 mt-3">
                              <TextArea
                                 ...
                                 placeholder={meta?.canChat ? 'Nhập tin nhắn của bạn...' : meta?.lockedReason || 'Chat đã bị khóa'}
                                 value={inputValue}
